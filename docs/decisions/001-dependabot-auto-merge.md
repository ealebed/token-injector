...
## Context
Dependabot opens daily PRs across gomod, Docker, GitHub Actions, and Terraform. Manual review of minor and patch bumps is slow and low-value. We still want humans to review major (and non-semver) updates, and we want CI to stay a merge gate.
...
- This repository is personal (`ealebed/token-injector`), so organization ruleset bypass lists are not available.
...
- Cons: Easy to miss a path Dependabot touches (Dockerfiles, workflow YAML, Terraform).
