...
type ServiceAccountInfo interface {
	GetEmail(context.Context) (string, error)
...
func (sa SaInfo) GetEmail(ctx context.Context) (string, error) {
	...
	email, err := metadataClient.EmailWithContext(ctx, "")
	if err != nil {
...
func (sa SaInfo) GetID(ctx context.Context) (string, error) {
	...
	availableScopes, err := metadataClient.ScopesWithContext(cx, "")
	if err != nil {
